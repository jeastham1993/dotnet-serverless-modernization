...
```bash
serverless-single-purpose$ sam build
```
...
```bash
serverless-single-purpose$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
serverless-single-purpose$ sam local start-api
serverless-single-purpose$ curl http://localhost:3000/
```
...
```bash
serverless-single-purpose$ sam logs -n HelloWorldFunction --stack-name serverless-single-purpose --tail
```
...
```bash
serverless-single-purpose$ dotnet test test/HelloWorld.Test
```
...
```bash
sam delete --stack-name serverless-single-purpose
```
